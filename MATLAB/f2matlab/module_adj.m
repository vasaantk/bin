% now put use module name in each subprg in that module
...
for i=size(sublist,1):-1:1
 if strcmp(sublist{i,4},'blockdata')
  %'heyyyyyyyyyyyyyy',kb
  %change blockdata's into sunroutines and remember for insertion into the main function later
  sublist{i,4}='subroutine';
  if isempty(sublist{i,1}) %empty blockdata, so give it a name
   sublist{i,1}='blockdata_empty';
  end % if isempty(sublist{i,
  funstr{sublist{i,9}}=[funstr{sublist{i,9}}(1:funstrwords_b{sublist{i,9}}(1)-1),...
                      'subroutine ',sublist{i,1}];
  funstr{sublist{i,3}}=regexprep(funstr{sublist{i,3}},'blockdata',['subroutine ',sublist{i,1}]);
  blockDataList{length(blockDataList)+1}=sublist{i,1};
