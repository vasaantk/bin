...
[writeErrFlag]=writeFmt(1,['%d','%d'],'''seed=''','seed');

[writeErrFlag]=writeFmt(1,['%g','%g'],'''rhs(1:)=''','rhs(:)');
...
[writeErrFlag]=writeFmt(1,['%g','%g'],'''x(1)=''','x(1)');
end; seed=fix(10+1);
...
func1result=[];
persistent a_old ab1as action catch_ml counter counter2 counter3 debug err exp_ml firstCall fmt fmtf fmti func1 fx_vec g i i1 i2 ipoint ix maxcol maxdig maxnam month my0 my1 my2 my3 n name names numnam onerpi p1 p2 p3 p4 p5 r0 r1 r2 r3 r4 r5 r6 radius1_test rval rvals seed seed2 sum_ml try_ml xi ; if isempty(month),month={};end; if isempty(names),names={};end; if isempty(xi),xi={};end; if isempty(firstCall),firstCall=1;end; 
...
for  i =( 1):( maxnam ),  if isempty(names), names ={  ' ' }; end; ;end;
 ...
 if isempty(sum_ml), sum_ml=zeros(1,p1); end;
 if isempty(exp_ml), exp_ml=0; end;
 if isempty(try_ml), try_ml=0; end;
 if isempty(catch_ml), catch_ml=0; end;
 ...
 if isempty(xi), xi=repmat({''},9,1); end;
...
try_ml=10;
catch_ml=fix(try_ml.*10);
sum_ml([2:-1:2]) = 20;
exp_ml=2.1;
[writeErrFlag]=writeFmt(1,['%g','%*g'],'''exp=''','exp_ml');
%print *,'exp(2.0)=',exp(2.0)
if( (strlexcmp( '1',  '0' ) >= 0) && (strlexcmp( 'a',  '9' ) <= 0) );
end;
rng('shuffle');
%write ( *, '(a,i6)' ) '    The value occurs in index ', index
[writeErrFlag]=writeFmt(1,['%2x','%6d',repmat([repmat('%14.6g',1,5)] ,1,1)],'maxdig','g([1:5],1)');
[writeErrFlag]=writeFmt(1,['%c','%c'],'''names(1)=''','names{1}');
err(:)=20;
[writeErrFlag]=writeFmt(1,['%c','%c'],'month{1}','month{2}');
[writeErrFlag]=writeFmt(1,['%c','%c'],'month{1}','month{2}');
g( 1, 1) =   1.0d+00;
...
[writeErrFlag]=writeFmt(1,['%g','%g'],'''a_old=''','a_old');
func1result=counter;
[writeErrFlag]=writeFmt(1,['%c','%*g'],'''shape(r1)=''','size(r1)');
[writeErrFlag]=writeFmt(1,['%c','%*g'],'''shape(r2)=''','size(r2)');
[writeErrFlag]=writeFmt(1,['%c','%*g'],'''shape(r3)=''','size(r3)');
[writeErrFlag]=writeFmt(1,['%c','%*g'],'''shape(err)=''','size(err)');
