...
if(n >= 0);
[writeErrFlag]=writeFmt(1,['%c','%c'],'''  n     Re[Dn(z)]       Im[Dn(z)]       ''','''Re[Dn''''(Z)]      IM[DN''''(Z)]''');
else;
[writeErrFlag]=writeFmt(1,['%c','%c'],''' -n     Re[Dn(z)]       Im[Dn(z)]       ''','''Re[Dn''''(Z)]      IM[DN''''(Z)]''');
[writeErrFlag]=writeFmt(1,['%c','%c'],''' -n     Re[Dn(z)]       Im[Dn(z)]       ''','''Re[Dn''''(Z)]      IM[DN''''(Z)]''');
end;
[writeErrFlag]=writeFmt(1,['%c','%c'],'''-------------------------------------------''','''-------------------------''');
for  i=0:n0;
...
end; if ~exist('tempBreak','var'),  m=250+1; end; clear tempBreak
  cdn=cb0.*cdn;
end;
end;
return;
...
end; if ~exist('tempBreak','var'),  k=16+1; end; clear tempBreak
      cdn=cb0.*cdn;
return;
