...
%%%if (strcmp('director',funstrwords{i}{temp2}))
%%% out,'uuuuuuuuuuu3',kb
%%%end      
   ...
   if size(varp,1)>0 
    ...
    if ~isempty(temp5)
     ...
     if ~isempty(goon)
      ...
      %'jjjjjjjjjjjjj',funstr{i},kb
%%%  if (strcmp('filter',funstrwords{i}{temp2}))
%%%   funstr{i},'uuuuuuuuuuu3444',kb
%%%  end      
  temp9={'',''};
  if want_ALLpoint, temp9{1}='mlPointer('; temp9{2}=')'; end
  if howmany==0
   goon=1;
   ...
   if strcmp(localVar{temp6,3},'character')
    if ischar(localVar{temp6,2}) && ~any(localVar{temp6,2}=='*')
  ...
  elseif howmany==1
   ...
   elseif strcmp(localVar{temp6,3},'character')% && length(localVar{temp6,5})>0
    out=[funstrwords{i}{temp2},'=repmat({''''},1,',char(localVar{temp6,5}),');'];
    ...
    if strcmpi(localVar{temp6,3},'logical'), temp8='false'; end
    if want_row
     out=[funstrwords{i}{temp2},'=',temp9{1},temp8,'(1,',temp7{1},temp9{2},');'];
    else
     out=[funstrwords{i}{temp2},'=',temp9{1},temp8,'(',temp7{1},',1)',temp9{2},';'];
    end
   end
  else
   if ~any(strcmp(localVar{temp6,3},var_words))
    ...
    if strcmp(localVar{temp6,3},'character')
     subscripts2='=repmat({''''},';
     %subscripts2='=cell(';
    elseif strcmpi(localVar{temp6,3},'logical')
     subscripts2=['=',temp9{1},'false('];
    else
     subscripts2=['=',temp9{1},'zeros('];
    end
   end
   for j=1:howmany
    if j~=howmany
     ...
     subscripts2=[subscripts2,temp7{j},')',temp9{2},';'];
    end
   end
   if ~any(strcmp(localVar{temp6,3},var_words))
...
%'sayyyyyyyyy',localVar,out,keyboard
 ...
 if howmany>0
  if strcmp(localVar{temp6,3},'character')
   out=[funstrwords{i}{temp2},...
        strrep(strrep(regexprep(funstr{i}(parens(2)+1:end),['^\*\d+'],''),...
                      '(/','{'),'/)','}')];
%%%   if strcmp(funstrwords{i}{temp2},'numb')
...
%%%            end
  ...
  out=[funstrwords{i}{temp2},...
       regexprep(funstr{i}(funstrwords_e{i}(temp2)+1:end),['^\*\d+'],'')];
