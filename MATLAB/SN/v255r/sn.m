...
if option == 1
fprintf('\n\t 1 --> Original runfile parameters\n\n')
filename = '/Users/satellite/Dropbox/bin/MATLAB/SN/v255r/oriRunfil_head.txt';
end
if option == 2
fprintf('\n\t 2 --> Original dataset aparm(3) = 1 (avg LL & RR)\n\n')
filename = '/Users/satellite/Dropbox/bin/MATLAB/SN/v255r/aparm3is1_head.txt';
end
...
month =    3 * ones(length(dayx),1);
day   =   18 * ones(length(dayx),1);
day(find(dayx)) = 19;
...
plot(tstmp(ATpos),mdel(ATpos)+(60E-9),ATmrk,tstmp(CDpos),mdel(CDpos)+(40E-9),CDmrk,tstmp(HOpos),mdel(HOpos)+(20E-9),HOmrk,tstmp(PApos),mdel(PApos),PAmrk)
legend('AT','CD','HO','PA')
